...
// Базовый шрифт 8x8 (ASCII 32-59: пробел, знаки препинания, цифры от 0 до 9)
...
static esp_err_t ssd1306_send_cmd(ssd1306_t *dev, uint8_t cmd)
{
    ...
    esp_err_t res = i2c_dev_write(&dev->i2c_dev, NULL, 0, data, 2);
...
esp_err_t ssd1306_init_desc(ssd1306_t *dev, i2c_port_t port, gpio_num_t sda_gpio, gpio_num_t scl_gpio)
{
    ...
    dev->i2c_dev.addr = SSD1306_I2C_ADDR;
...
{
    uint8_t cmds[] = {0xAE, 0x20, 0x02, 0xB0, 0xC8, 0x00, 0x10, 0x40, 0x81, 0x7F, 0xA1, 0xA6, 0xA8, 0x3F, 0xA4, 0xD3, 0x00, 0xD5, 0x80, 0xD9, 0x22, 0xDA, 0x12, 0xDB, 0x20, 0x8D, 0x14, 0xAF};
    for (size_t i = 0; i < sizeof(cmds); i++)
    {
        esp_err_t res = ssd1306_send_cmd(dev, cmds[i]);
        if (res != ESP_OK)
            return res;
    }
    return ESP_OK;
...
esp_err_t ssd1306_clear(ssd1306_t *dev)
{
    ...
    for (uint8_t page = 0; page < 8; page++)
    {
        ...
        I2C_DEV_TAKE_MUTEX(&dev->i2c_dev);
        i2c_dev_write(&dev->i2c_dev, NULL, 0, buf, 129);
        I2C_DEV_GIVE_MUTEX(&dev->i2c_dev);
    }
    return ESP_OK;
}
...
esp_err_t ssd1306_draw_string(ssd1306_t *dev, uint8_t x, uint8_t page, const char *str)
{
    ...
    ssd1306_send_cmd(dev, 0x10 | ((x >> 4) & 0x0F));
    ...
    while (*str)
    {
        char c = *str++;
        if (c < 32 || c > 127)
            c = ' '; // Заглушка, если символ вне массива font8x8
        memcpy(&payload[1], &font[c * 5], 5);

        I2C_DEV_TAKE_MUTEX(&dev->i2c_dev);
        i2c_dev_write(&dev->i2c_dev, NULL, 0, payload, 6);
        x += 6;
        if (x >= 128)
            break;
        I2C_DEV_GIVE_MUTEX(&dev->i2c_dev);
